...
if not isdir(TRAIN):
    TRAIN = '../../hubmap-organ-segmentation/train_images/'
...
class HuBMAPDataset(Dataset):
    def __init__(self, train=True, tfms=None):
        ...
        self.fnames = [fname for fname in os.listdir(TRAIN) if fname.split('.')[0] in ids or int(fname.split('.')[0]) in ids]
        self.train = train
        self.tfms = tfms
    ...
    def __getitem__(self, idx):
        data = self.labels[self.labels.id == self.ids[idx]]
